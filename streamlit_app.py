...
# Display the Landdrop image at the top of the page
...
with tab1:
    st.header("Summary of Chains Across All Wallets")
...
with tab2:
    st.header("Land Genesis Summary")
...
with tab3:
    ...
    if wallet_input:
        ...
        if not wallet_data.empty:
            ...
            wallet_data.columns = ['Wallet', '6', '5', '4', '3', 'Total Chains']
            
            # Display the table with custom headers
            st.table(wallet_data.set_index('Wallet'))
            
            # Total units calculation
            total_units = wallet_data[['6', '5', '4', '3']].sum(axis=1).values[0]

            st.markdown("<hr>", unsafe_allow_html=True)
            # Continue with the rest of the "Try your luck!" content
            st.subheader("Weights Sliders for Random Draw")
            st.write("The first slider adjusts the advantage for longer chains. The second slider illustrates how the chain size could impact how much of the advantage you get from the rarity slider. (When you run the random draw, the chain owned slider will not impact the results, rather the actual chain sizes in your wallet are used).")
            ...
            if st.button("Run Random Draw"):
                ...
                for idx, neighborhood in enumerate(unique_neighborhoods):
                    ...
                    if os.path.exists(image_path):
                        ...
                        cols[idx % 4].image(image, caption=neighborhood, use_column_width=True)
                    else:
